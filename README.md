...
- 새 버전끼리는 입력 메뉴 또는 다른 단축키로 바꾼 입력 소스 상태도 동기화됩니다.
- Bonjour로 Target을 자동 검색하고, 코드가 일치한 연결만 처리합니다.
- Target은 현재 입력 소스를 확인한 뒤 macOS API로 설정에서 고른 두 입력 소스를 직접 전환합니다.
- 일반 키 입력, 입력한 텍스트, 클립보드, 마우스 이벤트는 수집하거나 전송하지 않습니다.

### 현재 범위

- macOS 13 이상
- 설정에서 고른 임의의 두 입력 소스 조합 (기본값은 `ABC ↔ 두벌식`, 설정의 **전환할 입력 소스**에서 다른 언어 조합으로 바꿀 수 있습니다)
- 신뢰할 수 있는 동일 로컬 네트워크
- Source가 실행 중이면 현재 입력 소스 상태를 Target에 자동 전달
